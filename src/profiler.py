...
def analyse_csv_file(file_path) -> dict:
	...
	for column in columns_list:
		...
		table_metadata["columns"][column] = dict()
		...
		is_unique = bool(unique_count == row_count)
		has_nulls = bool(null_count > 0)
		has_id_pattern = bool("id" in column.lower())

		table_metadata["columns"][column]["dtype"] = str(df[column].dtype)
		table_metadata["columns"][column]["null_count"] = int(null_count)
		table_metadata["columns"][column]["null_ratio"] = float(null_ratio)
		table_metadata["columns"][column]["unique_count"] = int(unique_count)
		table_metadata["columns"][column]["unique_ratio"] = float(unique_ratio)
...
def analyse_all_csv(data_dir):
	...
	output_path = os.path.join(SRC_DIR, 'data_profile.json')
